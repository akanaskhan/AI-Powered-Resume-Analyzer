...
export const ResumeInsights = ({ insights }) => {
  ...
  const formatSection = (content) => {
    return content.split('\n').map((line, index) => {
      if (line.startsWith('##')) {
        return <h2 key={index} className="text-2xl font-semibold text-gray-800 mb-6">{line.replace('##', '').trim()}</h2>;
      }
      if (line.match(/^\*\*\d+\./)) {
        return <h3 key={index} className="text-xl font-semibold text-gray-800 mt-8 mb-4">{line.replace(/\*\*/g, '').trim()}</h3>;
      }
      if (line.trim().startsWith('*')) {
        if (line.includes('**')) {
          ...
          return (
            <div key={index} className="flex mb-4 ml-4 items-center justify-between">
      ...
      if (line.trim()) {
        return <p key={index} className="text-gray-600 font-light mb-4 leading-relaxed">{line.replace(/\*/g, '')}</p>;
      }
  ...
  return (
    <Card className=" bg-white shadow-md p-4 m-0   h-[660px] overflow-y-scroll">
      <div>
        <div className="text-center text-2xl text-gray-800">AI Resume Insights</div>
      </div>
      <div>
        {formatSection ? (
  <>
    {score !== null && (
      <div className="mb-6">
        <p className="text-sm font-medium text-gray-600 mb-1">Job Relevance Score</p>
  ...
  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-semibold text-black drop-shadow-sm">
    {score}%
        ...
        <p className="text-sm text-blue-600 mt-1 font-semibold">{score}/100</p>
      </div>
    )}
    {formatSection(insights)}
  </>
) : (
  "Your resume insights appears here.."
)}
      </div>
    </Card>
  );
